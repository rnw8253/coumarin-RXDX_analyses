...
for name in names:
        ...
        plot(fiber_tot_bend[:], fiber_tot_rot[:], "Rotation Angle ($\degree$)", "Bend Angle ($\degree$)", "%s.fiber" %(name),max=0.0005)
        plot(strand_tot_bend[:], strand_tot_rot[:], "Rotation Angle ($\degree$)", "Bend Angle ($\degree$)", "%s.strand" %(name),max=0.00038)
